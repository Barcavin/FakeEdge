...
def load_data(dataset, fuse, run=0):
    X_train = torch.load(f"{dataset}_Run_{run}_{fuse}_train_hidden_pos.pt",map_location='cpu')
    X_test = torch.cat([torch.load(f"{dataset}_Run_{run}_{fuse}_valid_hidden_pos.pt",map_location='cpu'),
                        torch.load(f"{dataset}_Run_{run}_{fuse}_test_hidden_pos.pt",map_location='cpu')])

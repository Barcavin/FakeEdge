...
def split_edges(data,args):
    ...
    split = RandomLinkSplit(num_val=args.val_ratio,
    ...
    data = Data(x=data.x, 
                ...
                train_pos=train.pos_edge_label_index,
                train_neg=train.neg_edge_label_index,
                val_pos=val.pos_edge_label_index,
                val_neg=val.neg_edge_label_index,
                test_pos=test.pos_edge_label_index,
                test_neg=test.neg_edge_label_index,
                num_nodes=num_nodes)
    return data
...
def plus_edge(data_observed, label, p_edge, args):
 ...
 edge_index = data_observed.edge_index, max_nodes_per_hop=args.max_nodes_per_hop ,num_nodes=data_observed.num_nodes)
    ...
    data.edge_mask_original = edge_mask.clone() # To get the original edges
...
def minus_edge(data_observed, label, p_edge, args):
 ...
 edge_index = data_observed.edge_index,max_nodes_per_hop=args.max_nodes_per_hop, num_nodes = data_observed.num_nodes)
    ...
    #edge_mask marks the edge under perturbation, i.e., the candidate edge for LP
    edge_mask = torch.ones(edge_index_p.size(1), dtype = torch.bool)
    ind1 = torch.where((edge_index_p == mapping.view(-1,1)).all(dim=0))
    edge_mask[ind1[0]] = False
    ind2 = torch.where((edge_index_p == mapping[[1,0]].view(-1,1)).all(dim=0))
    edge_mask[ind2[0]] = False
    if args.drnl == True:
        num_nodes = torch.max(edge_index_p)+1
    ...
    #label = 1 if the candidate link (p_edge) is positive and label=0 otherwise
    data.label = float(label)
    data.edge_mask_original = torch.ones_like(edge_mask, dtype=torch.bool) # To get the original edges
    return data
...
def load_unsplitted_data(args):
    ...
    data.num_nodes = torch.max(data.edge_index).item()+1
    return data
...
def set_init_attribute_representation(data,args):
    ...
    edge_index = data.edge_index # torch.cat((data.train_pos,data.train_pos[[1,0],:]),dim=1)
